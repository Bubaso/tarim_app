-- Hollanda dosyası içeriği.
--
-- Kaynak: supabase/seed/dossier_hollanda.sql (üretilmiş dosya).
-- Migration olarak taşınıyor çünkü içerik ürünün parçası: sıfırdan kurulan
-- her ortamda dizinin ilk sayısı bulunmalı. Betik yeniden çalıştırılabilir —
-- dosyayı upsert eder, bölümleri silip yeniden yazar.
--
-- İçteki begin/commit çıkarıldı: CLI her migration'ı zaten tek işlemde
-- çalıştırıyor, iç içe COMMIT sarmalayan işlemi erken kapatırdı.
