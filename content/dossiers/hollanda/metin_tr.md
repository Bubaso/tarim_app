# Hollanda — Ülke Dosyası · Türkçe metin

> **Tez:** Hollanda'nın sırrı toprağında değil, toprağının üstüne kurduğu sistemde.

---

## Yazım kuralları — bu dosyaya dokunan herkes için

1. **Hiçbir rakam hafızadan yazılmaz.** Her sayı `data.json`'dan gelir; `data.json`
   `_raw/`'dan, `_raw/` da adı geçen kurumun kendi yayınından. Zincir kırılırsa
   cümle silinir.
2. **Birim karıştırma yasağı.** Üretim değeri *sabit 2014-16 uluslararası doları*,
   ihracat *cari ABD doları*, CBS serileri *euro*. Bu üçü birbirine bölünmez.
   Karşılaştırma yalnızca **aynı ölçünün iki ülkedeki oranı** üzerinden yapılır.
3. **Doğrulanmamış kalem metne girmez.** `data.json → kurumlar._dogrulanmamis_kalemler`
   listesindeki hiçbir sayı burada kullanılamaz.
4. **Yıl her rakamın yanında.** Okuyucu hangi yılı okuduğunu cümleden anlamalı.
5. **Nedensellik iddiası kanıt ister.** Birlikte bulunan iki olgu "şu yüzden" diye
   bağlanmaz; "şununla birlikte" denir.

Kelime hedefi ~5.150. Bölüm başına ~400.
...
ihracatçısı nasıl olur?* Cevap toprakta değil. Toprağın üstüne kurulmuş olanda.
...
karşılığını beşinci bölümde göreceğiz.
...
suya daha az bağımlı. Girdisi daha yoğun. Coğrafya bu dosyayı açıklamıyor.
...
kısmına, ne yoğunlukta sermaye konduğu.
...
Bu, iklimle veya toprakla açıklanamayacak bir farktır. Bir tercih farkıdır.
...
değil. Beşi de kurumsal tasarım kararı. Ve bu yüzden beşi de kopyalanabilir.
...
sıra bir bilgi değil bir slogandır.
...
büyük kısmı, çiftçiden tüketiciye değil, fabrikadan fabrikaya akıyor.
...
**tavan** oldu. Ve tavana çarpan şey seralar değil, ahırlar.
